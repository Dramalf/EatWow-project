...
app.post('/addmeal', bodyParser.json({ limit: '10mb' }),async (request, response) => {
    ...
    console.log("2313",userid)
    await mongodb.updateOne('users', {  userid }, { $addToSet: { meals: JSON.stringify({ mealName, picUrl, tags, description }) } })
    tags.map(async(tag, index, arr) => {
       await mongodb.updateOne('users', { userid}, { $addToSet: { alltags: tag } })
       if (index === arr.length - 1) {
           let findData = await mongodb.find('users', { userid })
