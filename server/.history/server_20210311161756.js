...
app.post('/addmeal', bodyParser.json({ limit: '10mb' }),async (request, response) => {
    ...
    let d1=await mongodb.updateOne('users', { username:'mzy1' }, { $addToSet: { meals: JSON.stringify({ mealName, picUrl, tags, description }) } })
    console.log("$$",d1.ops[0])
    tags.map(async (tag, index, arr) => {
        await mongodb.updateOne('users', { _id:userid }, { $addToSet: { alltags: tag } })
        console.log(tag)
       if (index === arr.length - 1) {
           let findData = await mongodb.find('users', { _id:userid }).ops[0]
           console.log(userid)
