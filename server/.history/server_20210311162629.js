...
app.post('/addmeal', bodyParser.json({ limit: '10mb' }),async (request, response) => {
    ...
    mongodb.updateOne('users', { _id: `Object${userid}` }, { $addToSet: { meals: JSON.stringify({ mealName, picUrl, tags, description }) } })
    
    tags.map(async (tag, index, arr) => {
        mongodb.updateOne('users', { _id: `Object${userid}` }, { $addToSet: { alltags: tag } })
            .then(async() => {
                if (index === arr.length - 1) {
                    let findData = await mongodb.find('users', { _id: `Object${userid}` })
